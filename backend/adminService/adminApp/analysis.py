...
def test_confs(ratings_data, hdi_prob=0.94):
    ...
    results_df = pd.DataFrame({
        'ID': [str(i+1) for i in range(len(unique_configs))],
        'Название': unique_configs,
    ...
    best_config_name = f"{best_idx + 1} ({unique_configs[best_idx]})"
    ...
    significantly_worse = [
        f"{i+1} ({config})" for i, (config, low) in enumerate(zip(unique_configs, lower_quantiles))
    ...
    fig_bar.update_traces(
        hovertemplate="<br>".join([
            ...
            "Апостериорное среднее: %{y:.3f}",
            "P(лучшая конфигурация): %{marker.color:.3f}",
            "<extra></extra>"
        ])
    )
    ...
    for i, row in results_df.iterrows():
        fig_forest.add_trace(go.Scatter(
            ...
            showlegend=False,
            customdata=[[row['Название']] * 2],
            hovertemplate="<br>".join([
                "ID: %{y}",
                "Название: %{customdata[0]}",
                "Доверительный интервал: [%{x[0]:.3f}, %{x[1]:.3f}]",
                "<extra></extra>"
            ])
        ))
        fig_forest.add_trace(go.Scatter(
            ...
            showlegend=False,
            customdata=[[row['Название']]],
            hovertemplate="<br>".join([
                "ID: %{y}",
                "Название: %{customdata[0]}",
                "Апостериорное среднее: %{x:.3f}",
                "<extra></extra>"
            ])
        ))
    fig_forest.update_layout(
        ...
        xaxis_title='Оценка',
        yaxis_title='ID конфигурации',
        template='plotly_white'
    )

    # Создаем таблицу с правильными заголовками
    table_headers = [
        'ID конфигурации',
        'Название конфигурации',
        'Апостериорное среднее',
        f'Нижний квантиль ({int((1-hdi_prob)/2*100)}%)',
        f'Верхний квантиль ({int((1+(hdi_prob))/2*100)}%)',
        'Вероятность быть лучшей'
    ]
    ...
    fig_table = go.Figure(data=[go.Table(
        ...
        cells=dict(
            values=[
                results_df['ID'],
                results_df['Название'],
                results_df['Апостериорное среднее'].round(4),
                results_df[f'Нижний квантиль ({int((1-hdi_prob)/2*100)}%)'].round(4),
                results_df[f'Верхний квантиль ({int((1+(hdi_prob))/2*100)}%)'].round(4),
                results_df['P(лучшая конфигурация)'].round(4)
            ],
            fill_color='white',
            align='left',
            font=dict(size=11)
    ...
    fig_table.update_layout(
        title='Апостериорная таблица по конфигурациям',
